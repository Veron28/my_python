...
edit: function() {
  ...
  if (wkcur.type == 'wall') {
    if (wkcur.edit) {
      ...
      Wall.editPost(ge('wpe_edit' + wkcur.post), wkcur.post, {from: 'wkview'});
    }
    return true;
  }
...
onScroll: function(ev, resize) {
  ...
  if (resize && !wkcur.fixedBottom && wndHeight - ph < py + 20) {
    ...
    if (!wkcur.fixedBottom || resize) {
      ...
      setStyle(buttons, {width: buttonsSize[0], height: buttonsSize[1]});
      setStyle(buttonsCont, {paddingTop: buttonsSize[1]});
    }
  } else {
    if (wkcur.fixedBottom || resize) {
      wkcur.fixedBottom = false;
      removeClass(wkcur.wkContent, 'wke_bottom_fixed');
      setStyle(buttons, {width: null, height: null});
      setStyle(buttonsCont, {paddingTop: 0});
    }
  }
  ...
  if (controlsCont) {
    ...
    if (pos[1] < 0) {
      if (!wkcur.fixedTop) {
        ...
        setStyle(controls, {width: controlsSize[0], height: controlsSize[1]});
        setStyle(controlsCont, {paddingTop: controlsSize[1]});
      }
    } else {
      if (wkcur.fixedTop) {
        wkcur.fixedTop = false;
        removeClass(wkcur.wkContent, 'wke_top_fixed');
        setStyle(controls, {width: null, height: null});
        setStyle(controlsCont, {paddingTop: 0});
      }
    }
  }
},
...
initWkBox: function(title, html, options, script) {
  if (curBox() && curBox().type && curBox().type == options.type) {
    curBox().setOptions({onHide: false});
    curBox().hide();
  }

  var box = showFastBox(extend({title: title}, options), html);
  if (options.wkRaw) {
  ...
  box.setOptions({onHide: function(noLoc) {
    if (!noLoc && nav.objLoc.w) {
      nav.change({'w': false}, undefined, { asBox: true });
    }
  }, onShow: function() {
    if (box.wkRaw) {
  ...
  var nl = extend(nav.objLoc, {'w': options.wkRaw});
  delete(nl.z);
...
<div id="wk_right_arrow_bg" class="wk_arrow_bg no_select" onclick="return WkView.navigate(this, event, 1);" onmouseover="WkView.preloadArrow(true)"><div class="wk_arrow_bg_inner"></div><div id="wk_right_arrow" class="wk_arrow no_select"></div></div>';
    ...
    if (options.wkRaw.substr(0, 4) == 'away') {
      ...
      setTimeout(function() {
        if (window._mediator) {
          window._mediator.start({ url: location.pathname + location.search });
        }
      }, 1500);
    }
  }
...
updateArrows: function() {
  ...
  var arrowActions = WkView.getNextWkRaws(), intro = (wkcur.wkRaw == 'intro');
  if (arrowActions[0] || arrowActions[1] || intro) {
    ...
    setStyle(wkcur.wkRight, {paddingBottom: (arrowTop - getXY(wkcur.wkRight, true)[1] - 24) / 2});
...
navigate: function (el, event, delta) {
  ...
  if (delta > 0 && actions[1]) {
    ...
    showWiki(extend(options, {w: actions[1]}), false, false, {fromlist: 1, noloader: true, preload: {cache: wkcur.navNoCache || WkView.recacheRaw(actions[1]) ? 0 : 1 }});
    cancelEvent(event);
  }
  if (delta < 0 && actions[0]) {
    ...
    showWiki(extend(options, {w: actions[0]}), false, false, {fromlist: -1, noloader: true, preload: {cache: wkcur.navNoCache || WkView.recacheRaw(actions[0]) ? 0 : 1 }});
    cancelEvent(event);
  }
},
...
extPageSubscribe: function(btn, oid, hash, confirm, transition_source, ad_data) {
  ...
  var subscribed = hasClass(btn, 'secondary'), reqOptions = {
  ...
  if (oid > 0) {
    ajax.post('al_friends.php', {act: (subscribed ? 'remove' : 'add'), mid: oid, hash: hash, from: 'wkview_extpage'}, reqOptions);
  } else {
    var leave = ajax.post.pbind('al_groups.php', {act: (subscribed ? 'a_leave' : 'a_enter'), gid: -oid, hash: hash, from: 'wkview_extpage', t_src: transition_source, ad_data: ad_data || ''}, reqOptions);
    if (confirm && subscribed) {
...
wallBeforeInitPost: function (opts) {
  ...
  while (true) {
    ...
    postPlaceholder = ce('div', {className: 'wk_wall_post_placeholder', id: 'wk_wall_post_placeholder_' + postEl.id});
...
wallPostDelete: function(act, hash, force) {
  ...
  ajax.post('al_wall.php', {act: act, post: post, hash: hash, confirm: force ? 1 : 0, from: 'wkview'}, {onDone: function(msg, additional, need_confirm) {
    ...
    if (del && hasClass(del, 'no_rows')) {
      ...
      domPN(p).insertBefore(ce('div', {id: 'post_del' + post, innerHTML: msg, className: 'no_rows'}), domNS(p));
      hide(p);
    }
    wkLayerWrap.scrollTop = 0;
    if (act == 'spam') {
...
wallUpdateRepliesOnScroll: function (resize) {
  ...
  if (resize && wkcur.fixedBottom === false && wndHeight - formH < formY + 20) {
    ...
    setStyle('wl_reply_form_wrap', {width: formSize[0], height: formSize[1], bottom: bottom, marginLeft: ml});
    ...
    if (wkcur.fixedBottom || resize) {
      ...
      setStyle('wl_reply_form_wrap', {width: null, height: null, marginLeft: null});
    }
  }
},
...
shareApp: function (app_url, url_hash) {
  if (vk.id) {
    var params = {act: 'publish_box', object: wkcur.like_obj, url_hash: url_hash, list: '', from: 'wkview'};
    var options = {
      onDone: function() {
        var url = app_url;
        if (url_hash) {
