var Privacy = {
  flistBox: function (box, friendsData, selectedFriends, rowTemplate, saveLang, hash) {
    ...
    if (cur.flistTooltip) {
      ...
      stManager.add(['tooltips.js'], function() {
        box.setOptions({onHide: tooltips.hide.pbind(targetEl)});
      });
    } else {
  ...
  customType: 4, // все, кроме
  someType: 5, // некоторые друзья
  listsType: 6, // некоторые списки друзей
  /**
   * @param {String} key
  ...
  choose: function(ev, val, list, confirmed) {
    ...
    if (val == Privacy.customType) {
      ...
      return showTabbedBox('al_friends.php', extend({}, cur.privacy.chooseBoxOpts || {}, { act: 'select_friends_box', Checked: checked, pagination: cur.privacy.pagination ? 1 : '' }), { stat: ['ui_controls.js'] });
  ...
  show: function(el, ev, key, delta) {
    ...
    if (!cur.privEl) {
      cur.privEl = elWrap.appendChild(ce('div', {id: 'privacy_dropdown', innerHTML: '<div class="rows"></div>'}));
      cur.privRows = cur.privEl.firstChild;
