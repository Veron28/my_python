...
scrollResize: function () {
  ...
  var docEl = document.documentElement,
      ...
      st = scrollGetY(), lnkY = getXY(lnk)[1];
...
getDocHTML: function(item) {
  // параметры были записаны в hDocsList
  var id = item[0],
      ext = item[1],
      titleAttr = item[2], title = titleAttr,
      dateStr = item[3],
      oid = item[4],
      tags = item[6],
      mid = item[7],
  ...
  if (tags) {
    ...
    tags = rs(cur.tpl.tags, {tags: tags.join(', ')});
  }
  var thumb = item[8],
...
globalSearch: function(more) {
  ...
  cur.searchTimeout = setTimeout(function() {
    ...
    ajax.post('/docs.php', {act: 'search_docs', q: cur.searchStr, offset: more && cur.searchShown || 0, oid: cur.oid}, options);
  }, more ? 10 : 300);
...
editItem: function(el, oid, did) {
  return !showBox('/docs.php', {act: 'edit_box', oid: oid, did: did});
},
   ...
   * @param oid
   * @param did
   * @param fragment bool - форсировать ли загрузку демо-файла, если это лицензионный файл
   * @param event
   * @returns {*}
   */
downloadItem: function(el, oid, did, fragment, event) {
  ...
  if (!item) {
    if (domClosest('_feed_notification', el) && cur && cur.section == 'notifications') { // хак, позвол€ющий в ответах не открыватьс€ посту и ссылке одновременно
      return false;
    }
    return true;
  }
...
addItem: function(el, oid, did, hash) {
  ...
  if (!info) {
    var params = {act: 'a_add', doc: fullId, hash: hash, to_id: (cur.groupAdmin ? cur.oid : vk.id)};
    ...
    var params = {act: 'a_delete', hash: hash, did: info[0], oid: info[4]};
    ajax.post('/docs.php', params, {
      onDone: function() {
...
//ѕоддерживаем удаление модераторами, дл€ этого нужны sure и comment
deleteItem: function(el, oid, did, hash, sure, comment) {
  var doc = domClosest('_docs_item', el);
  if (!doc) return;

  // Ќужно провер€ть наличие window.tooltips. т.к удаление может быть ининциировано со скринр-ридеров, где не будет
  // ¬ызван showTooltip и не произойдет добавление статики через stManager.
  el && window.tooltips && tooltips.destroy(el);
  ajax.post('/docs.php', {act: 'a_delete', hash: hash, did: did, oid: oid, sure: sure, comment: comment}, {
    onDone: function(text) {
      if (!text) {
        stManager.add([jsc('internal/group_info.js'), 'groupinfo.css'], function() {
          window.createConfirmBox(Docs.deleteItem.bind(null, el, oid, did, hash, 1), "документ");
        });
...
selectTag: function(obj) {
  ...
  cur.section = 'all';
  cur.type = '';
  return Docs.tagSearch(obj, {from: 'menu'});
},
...
rowActive: function(obj, type) {
  ...
  if (ttText) {
    showTooltip(obj, {text: function() {
      return ttText;
    }, showdt: 500, black: 1, shift: shift});
  }
},
  ...
  each(files, function() {
    if (this.name.substr(-3).toLowerCase() !== 'mp3') {
      ...
      if (errCont) {
        ...
        setTimeout(showFastBox({title: getLang('global_error')}, getLang('docs_upload_mp3_fail')).hide, 4000);
      }
    }
  });
...
chooseSwitch: function(obj) {
  ...
  ajax.post('/docs.php', {act: 'a_choose_doc_box', offset: 0, to_id: cur.docsToId, tab: cur.docsTab, switch_tab: 1, ext_filter: extFilter || ''}, {
...
chooseMore: function() {
  ...
  ajax.post('/docs.php', {act: 'a_choose_doc_box', offset: cur.docsOffset, to_id: cur.docsToId, tab: cur.docsTab, q: cur.docsSearchStr, more: 1, ext_filter: cur.docsCurFilter}, {
...
chooseUploadFailed: function(info, code) {
  ...
  if (Upload.types[i] == 'fileApi' && !Upload.options[i].wiki_editor) {
    ...
    if (showBox) {
      setTimeout(showFastBox({title: getLang('global_error')}, errMsg).hide, 4000);
    }
  }
  if (curBox()) {
...
onChooseDragEnter: function(ev) {
  ...
  var box = curBox(), body = box.bodyNode;
  if (isVisible(cur.docsChooseRows)) {
    box.bodyH = getSize(body)[1];
  } else if (box.bodyH !== undefined) {
    var bodyY = getXY(body, true)[1], dropboxH = getSize(cur.docsChooseDropbox)[1],
